//以下のコードは、Google Apps Script内のコードです
...
function mergeAndShowF_batch() {
    ...
    // トリガー条件チェック: 対象範囲の編集時のみ実行
    const editedRange = sheet.getActiveRange();
    if (editedRange) {
        const editedCol = editedRange.getColumn();
        // B列(2)またはR列(18)の編集時のみ実行
        if (editedCol !== 2 && editedCol !== 18) {
            console.log('対象外の列の編集のため処理をスキップ');
            return;
        }
    }
    ...
    // 必要なデータを一度に取得（パフォーマンス改善）
    const allData = sheet.getRange(startRow, dateCol, numRows, rCol - dateCol + 1).getValues();
    const dates = allData.map(row => row[0]); // B列
    const rValues = allData.map(row => row[rCol - dateCol]); // R列
    ...
    for (let i = 0; i <= numRows; i++) {
        ...
        // R列の値を分に変換（エラーハンドリング強化）
        let minutes = 0;
        if (rValue) {
            try {
                const timeStr = rValue.toString();
                if (timeStr.includes(':')) {
                    minutes = timeStr.split(":").reduce((h, m) => h * 60 + parseInt(m, 10), 0);
                }
            } catch (error) {
                console.log(`時間変換エラー: ${rValue}`);
            }
        }
    ...
    console.log(`mergeAndShowF_batch実行時間: ${executionTime}ms (処理行数: ${numRows}, 更新セル: ${updates.length})`);
}

// ==========================
// 最高速版の出勤簿処理関数（極限最適化）
// ==========================
function mergeAndShowF_ultraFast() {
    const startTime = Date.now();
    ...
    if (editedRange) {
        ...
        // 対象列かつ対象行範囲内の編集時のみ実行
        if ((editedCol !== 2 && editedCol !== 18) || editedRow < 7 || editedRow > 88) {
            console.log('対象外の範囲編集のため処理をスキップ');
            return;
        }
    }
    ...
    // 必要最小限のデータ取得
    const dataRange = sheet.getRange(config.startRow, config.dateCol, config.maxRow - config.startRow + 1, config.rCol - config.dateCol + 1);
    ...
    if (lastDataRow === -1) return; // データなし

    const actualRows = lastDataRow + 1;

    // F列の一括クリア（必要な範囲のみ）
    const fRange = sheet.getRange(config.startRow, config.fCol, actualRows, 1);
    fRange.breakApart().clearContent().setBorder(true, true, true, true, true, true);
    ...
    for (let i = 0; i < actualRows; i++) {
        const dateValue = allData[i][0];
        const timeValue = allData[i][config.rCol - config.dateCol];

        if (!currentGroup || currentGroup.date !== dateValue) {
            if (currentGroup && currentGroup.totalMinutes > 0) {
        ...
        if (timeValue && typeof timeValue === 'string' && timeValue.includes(':')) {
            ...
            currentGroup.totalMinutes += parseInt(h, 10) * 60 + parseInt(m, 10);
        }
    }
    ...
    mergeGroups.forEach(group => {
        const rowCount = group.endRow - group.startRow + 1;
        const range = sheet.getRange(config.startRow + group.startRow, config.fCol, rowCount, 1);

        // 結合が必要な場合のみ追加
        if (rowCount > 1) {
            mergeOperations.push(() => range.mergeVertically());
        }
        ...
        valueOperations.push(() => range.setValue(timeString));
    });

    // 一括実行
    mergeOperations.forEach(op => op());
    valueOperations.forEach(op => op());

    const endTime = Date.now();
    console.log(`ultraFast実行時間: ${endTime - startTime}ms (処理行数: ${actualRows}, グループ数: ${mergeGroups.length})`);
}
...
function performanceDiagnostic() {
    const sheet = SpreadsheetApp.getActiveSpreadsheet().getActiveSheet();
    if (sheet.getName() !== "出勤簿") {
        console.log('出勤簿シートで実行してください');
        return;
    }

    console.log('=== パフォーマンス診断開始 ===');

    // 1. オリジナル版のテスト（データのバックアップ後）
    console.log('1. 通常版テスト開始');
    mergeAndShowF_batch();

    Utilities.sleep(1000); // 1秒待機

    // 2. 高速版のテスト
    console.log('2. 高速版テスト開始');
    mergeAndShowF_ultraFast();

    console.log('=== パフォーマンス診断完了 ===');
}

// ==========================
// エラー監視とログ機能
// ==========================
function errorMonitoredMerge() {
    try {
        // まず高速版を試行
        mergeAndShowF_ultraFast();
    } catch (error) {
        console.error('高速版でエラー発生、通常版にフォールバック:', error);
        try {
            // エラー時は通常版にフォールバック
            mergeAndShowF_batch();
        } catch (fallbackError) {
            console.error('通常版でもエラー発生:', fallbackError);
            // 管理者に通知するロジックを追加可能
        }
    }
}
...
function handleFormSubmission(e) {
  try {
    ...
    if (recordSheet.getLastRow() === 0) {
      ...
      // ヘッダーのスタイル設定
      const headerRange = recordSheet.getRange(1, 1, 1, 14);
    ...
    const rowData = [
      ...
      formData.nextVisitDate || '', // 次回通院日
      formData.nextVisitTime || '', // 次回通院時刻
      formData.hospitalName || ''   // 次回病院名
    ];

    recordSheet.getRange(newRow, 1, 1, 14).setValues([rowData]);
